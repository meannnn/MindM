...
def create_optimized_template():
    ...
    # 使用docxtpl的表格插入语法
    # 这里我们创建一个占位符，实际的数据会在模板处理器中填充
    doc.add_paragraph('{% for activity in learning_activities %}')
    doc.add_paragraph('| {{ activity.name }} | {{ activity.teacher_activity }} | {{ activity.student_activity }} | {{ activity_intent }} |')
    ...
    # 使用docxtpl的表格插入语法
    doc.add_paragraph('{% for point in reflection_thinking_points %}')
    doc.add_paragraph('| {{ point.point_type }} | {{ point.description }} |')
    doc.add_paragraph('{% endfor %}')
    
    # 保存文档
    output_path = 'templates/optimized_teaching_design_template.docx'
    doc.save(output_path)
    print(f"优化模板文件已创建: {output_path}")
