...
def create_table_template():
    ...
    # 创建学习活动表格模板
    activities_table = doc.add_table(rows=2, cols=4)  # 包含表头和一行示例
    ...
    # 在第二行添加docxtpl循环语法
    # 这里我们需要手动设置表格行的内容
    example_row = activities_table.rows[1]
    example_row.cells[0].text = '{% for activity in learning_activities %}{{ activity.name }}{% endfor %}'
    example_row.cells[1].text = '{% for activity in learning_activities %}{{ activity.teacher_activity }}{% endfor %}'
    example_row.cells[2].text = '{% for activity in learning_activities %}{{ activity.student_activity }}{% endfor %}'
    example_row.cells[3].text = '{% for activity in learning_activities %}{{ activity.activity_intent }}{% endfor %}'
    ...
    # 在第二行添加docxtpl循环语法
    thinking_example_row = thinking_table.rows[1]
    thinking_example_row.cells[0].text = '{% for point in reflection_thinking_points %}{{ point.point_type }}{% endfor %}'
    thinking_example_row.cells[1].text = '{% for point in reflection_thinking_points %}{{ point.description }}{% endfor %}'
    
    # 保存文档
    output_path = 'templates/table_teaching_design_template.docx'
    doc.save(output_path)
    print(f"表插入模板文件已创建: {output_path}")
