...
{{
    ...
    "learning_objectives": "学习目标及重难点（3-4个具体可测量的目标，使用行为动词，标注重点和难点，必须采用序号形式呈现）",
...
5. **学习目标**：3-8个具体可测量的目标，使用行为动词（如"列举"、"对比"、"设计"、"总结"），避免模糊词汇（如"了解"、"知道"、"掌握"），标注重点和难点。**重要**：学习目标必须以字符串格式输出，每行一个目标，必须采用序号形式呈现，格式如下：
